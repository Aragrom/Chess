...
public class KingLogic : PieceLogic
{
    ...
    public LaneLogic north = null;
    public LaneLogic south = null;
    public LaneLogic east = null;
    public LaneLogic west = null;
    public LaneLogic northEast = null;
    public LaneLogic northWest = null;
    public LaneLogic southEast = null;
    public LaneLogic southWest = null;
    ...
    public void CheckForCheck()
    {
        ...
        if (piecesAttackingThisKing != null
            && piecesAttackingThisKing.Length > 0)
        {
            inCheck = true;
        }
        else
    ...
    public List<int> GetLaneSupportingPieces(LaneLogic lane, int[] exceptions, List<int> allSupportingPieces)
    {
        int counter = 0;
    ...
    public LaneLogic AddDiagonalVulnerabilities(int step,
        LaneLogic lane)
    {
        ...
        {
            lane.state = LaneLogic.State.OffBoard;
        }
        ...
        while (board.IsMoveInScope(tempIndex))
        {
            ...
            else
            {
                ...
                {
                    if (lane.state == LaneLogic.State.Open)
                    {
                        ...
                        if (board.DoesSquareHavePiece(tempIndex))
                        {
                            ...
                            if (board.IsWhiteControlled(tempIndex) == isWhite)
                            {
                                ...
                                lane.state = LaneLogic.State.Defended;
                            ...
                            else
                            {
                                ...
                                if (board.squares[tempIndex].piece.type == PieceLogic.Type.Bishop
                                    || board.squares[tempIndex].piece.type == PieceLogic.Type.Queen
                                    ||
                                    (board.squares[tempIndex].piece.type == PieceLogic.Type.Pawn
                                    && (((PawnLogic)board.squares[tempIndex].piece).promotionType == PieceLogic.Type.Bishop
                                    || ((PawnLogic)board.squares[tempIndex].piece).promotionType == PieceLogic.Type.Queen))
                                    )
                                {
                                    lane.state = LaneLogic.State.Attacked;
                                ...
                                else
                                {
                                    ...
                                    if (board.squares[tempIndex].piece.type == PieceLogic.Type.Pawn
                                        && depth == 1)
                                    {
                                        lane.state = LaneLogic.State.Attacked;
                                        lane.attackerDepth = depth;
                                    ...
                                    {
                                        lane.state = LaneLogic.State.Blocked;
                        ...
                        else
                        {
                            ...
                            lane.state = LaneLogic.State.Open;
                        }
                    }
                    else
                    {
                        ...
                        if (board.DoesSquareHavePiece(tempIndex))
                        {
                            ...
                            if (board.IsWhiteControlled(tempIndex) == isWhite)
                            {
                                ...
                                lane.state = LaneLogic.State.DoubleDefence;
                                lane.blockerDepth = depth;
                                lane.blocker = tempIndex;
                                hasLaneInfo = true;
                            }
                            else
                            {
                                ...
                                if (lane.state == LaneLogic.State.Defended)
                                {
                                    if (board.squares[tempIndex].piece.type == PieceLogic.Type.Bishop
                                    || board.squares[tempIndex].piece.type == PieceLogic.Type.Queen
                                    ||
                                    ...
                                    {
                                        lane.state = LaneLogic.State.DefendedAttack;
                                        lane.attackerDepth = depth;
                                        lane.attacker = tempIndex;
                                        hasLaneInfo = true;
                                    }
                                    ...
                                    {
                                        lane.state = LaneLogic.State.DefendedMove;
                                        lane.blockerDepth = depth;
                                        lane.blocker = tempIndex;
                                        hasLaneInfo = true;
                                    }
    ...
    public LaneLogic AddVerticalVulnerabilities(int step,
        LaneLogic lane)
    {
        List<int> tempSquares = new List<int>();
                    ...
                    if (lane.state == LaneLogic.State.Open)
                    {
                        ...
                        if (board.DoesSquareHavePiece(tempIndex))
                        {
                            ...
                            else
                            {
                                ...
                                if (board.squares[tempIndex].piece.type == PieceLogic.Type.Rook
                                    || board.squares[tempIndex].piece.type == PieceLogic.Type.Queen
                                    ||
                                    (board.squares[tempIndex].piece.type == PieceLogic.Type.Pawn
                                    && (((PawnLogic)board.squares[tempIndex].piece).promotionType == PieceLogic.Type.Rook
                                    || ((PawnLogic)board.squares[tempIndex].piece).promotionType == PieceLogic.Type.Queen))
                                    )
                                {
                                    lane.state = LaneLogic.State.Attacked;
    ...
    public LaneLogic AddHorizontalVulnerabilities(int step,
        LaneLogic lane)
    {
        List<int> tempSquares = new List<int>();
